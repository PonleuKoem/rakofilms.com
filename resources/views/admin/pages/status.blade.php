      ...
      <form action="{{URL::to('create/search')}}">
        ...
        <tbody>
          <tr>
            ...
            <td class="">{{$key+1}}</td>
            <td class="">{{ str_limit($row->status, 20)}}</td>
                ...
                <div class="modal-dialog">
                  <div class="modal-content">
                    ...
                    <button type="button" class="btn btn-default  pull-left" data-dismiss="modal">Close</button>
                       {{ Form::open(array('url' => 'cms/statuses/' . $row->id, 'class' => 'pull-right')) }}
                ...
                <div class="modal-dialog">
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-footer">
                       ...
                       <form action="{{URL::to('/cms/statuses')}}" method="POST" enctype="multipart/form-data">
                          {{csrf_field() }}
                          <div class="form-group">
                            <label name="status" class="control-label">status:</label>
                            <input type="text" value="{{ old('status') }}" class="form-control" name="status" placeholder="status" required="required" pattern="([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?" value="{{old('status')}}" title="Enter a valid status"/><span style="color: red">{{ ($errors -> has('status')) ? $errors -> first('status') : ''}}</span>
                          </div>
